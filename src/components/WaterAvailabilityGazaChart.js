...
const WaterAvailabilityGazaChart = () => {
  const [plotData, setPlotData] = useState(null);
  const [plotLayout, setPlotLayout] = useState(null);
  const [plotFrames, setPlotFrames] = useState(null);

  useEffect(() => {
    axios.get('https://dataviz-website-restless-sound-977.fly.dev/water_availability_gaza_plot')
      .then(response => {
...
export default WaterAvailabilityGazaChart;
